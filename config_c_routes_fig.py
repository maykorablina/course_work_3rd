...
def main():
    wm, orders = make_wm()
    central_aisle_slotting(wm, orders)
    ...
    draw(axes[0, 0], wm, p_o0_bfs,
         f"$o_0$ — CAS + BFS: {len(p_o0_bfs)} steps")
    draw(axes[0, 1], wm, p_o0_ca,
         f"$o_0$ — CAS + Dijkstra-NN ($\\lambda=5$): {len(p_o0_ca)} steps")
    draw(axes[1, 0], wm, p_typ_bfs,
         f"Typical — CAS + BFS: {len(p_typ_bfs)} steps")
    draw(axes[1, 1], wm, p_typ_ca,
         f"Typical — CAS + Dijkstra-NN ($\\lambda=5$): {len(p_typ_ca)} steps")
    fig.suptitle("Routes on CAS-slotted warehouse: BFS vs Dijkstra-NN",
                 fontsize=13, fontweight="bold", y=0.995)
    plt.tight_layout()
    out = "final/fig_config_c_routes.png"
