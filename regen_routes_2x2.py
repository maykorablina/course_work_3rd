...
def draw_panel(ax, model, path, title):
...
def main():
    wm, orders = make_wm()
    typ_items = [it for it, _ in Counter(it for o in orders for it in o).most_common(ITEMS_PER_ORDER)]
    o0 = orders[0]

    p_o0_bfs = wm._route_order_silent(o0)
    p_o0_ca  = wm._route_order_congestion_silent(o0,  lam=LAM)
    p_typ_bfs = wm._route_order_silent(typ_items)
    p_typ_ca  = wm._route_order_congestion_silent(typ_items, lam=LAM)

    fig, axes = plt.subplots(2, 2, figsize=(14, 14))
    draw_panel(axes[0, 0], wm, p_o0_bfs,
               f"Order $o_0$ — Baseline (distance-sort+BFS): {len(p_o0_bfs)} steps")
    draw_panel(axes[0, 1], wm, p_o0_ca,
               f"Order $o_0$ — Config A (Dijkstra-NN, $\\lambda=5$): {len(p_o0_ca)} steps")
    draw_panel(axes[1, 0], wm, p_typ_bfs,
               f"Typical order — Baseline: {len(p_typ_bfs)} steps")
    draw_panel(axes[1, 1], wm, p_typ_ca,
               f"Typical order — Config A: {len(p_typ_ca)} steps")
    fig.suptitle("Picking routes: Baseline vs Config A (Dijkstra-NN)",
                 fontsize=13, fontweight="bold", y=0.995)
    plt.tight_layout()
    out = "final/fig_routes_2x2.png"
